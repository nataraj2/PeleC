          ...
          for (int jj = -1; jj <= 1; jj++) {
            for (int kk = -1; kk <= 1; kk++) {
              if (
                ...
                if (
                  (as_crse_crse_cell) and
                  (rr_flag_crse(iii, jjj, kkk) == amrex_yafluxreg_fine_cell) and
...
void
redistribute_negative_mass_fractions(
	const amrex::Box bx, 
	amrex::Array4<amrex::Real> const& s)
{
	amrex::ParallelFor(
		...
		s(i,j,k,ns + UFS) = std::max(0.0,s(i,j,k,ns + UFS));
		}
	});	
        };
